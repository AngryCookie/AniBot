...
from bot.ui import ConfirmView, ReasonModal, build_ux_embed
...
class ModerationCog(commands.Cog):
    ...
    async def _log_action(self, guild_id: int, action: str, moderator_id: int, user_id: int | None = None, reason: str = "") -> None:
        async with self.bot.db.session() as session:
            session.add(ModLog(guild_id=guild_id, action=action, moderator_id=moderator_id, user_id=user_id, reason=reason))
    ...
    async def warn(self, interaction: discord.Interaction, member: discord.Member, reason: str) -> None:
        async with self.bot.db.session() as session:
            session.add(Warning(guild_id=interaction.guild.id, user_id=member.id, moderator_id=interaction.user.id, reason=reason))
            await session.commit()
    ...
    async def warnings(self, interaction: discord.Interaction, member: discord.Member) -> None:
        async with self.bot.db.session() as session:
            rows = await session.execute(select(Warning).where((Warning.guild_id == interaction.guild.id) & (Warning.user_id == member.id)))
            warns = rows.scalars().all()
        if not warns:
            await interaction.response.send_message("Предупреждений нет.", ephemeral=True)
            return
        embed = discord.Embed(title=f"Предупреждения {member.display_name}", description="\n".join([f"#{w.id} {w.reason} ({w.created_at.date()})" for w in warns]))
    ...
    async def mute(self, interaction: discord.Interaction, member: discord.Member, reason: str = "") -> None:
        if not reason:
            modal = ReasonModal(title="Причина для mute")
            await interaction.response.send_modal(modal)
            await modal.wait()
            reason = modal.value
        embed = build_ux_embed(title="🛡️ Подтвердите mute", description=f"Пользователь: {member.mention}\nПричина: {reason or 'не указана'}", color=discord.Color.orange(), next_hint="После подтверждения роль Muted будет назначена.")
        ...
        async def _confirm(i: discord.Interaction) -> None:
            ...
            await i.response.edit_message(content="✅ Пользователь замьючен.", embed=None, view=None)
    ...
    async def kick(self, interaction: discord.Interaction, member: discord.Member, reason: str = "") -> None:
        embed = build_ux_embed(title="⚠️ Подтвердите kick", description=f"Пользователь: {member.mention}\nПричина: {reason or 'не указана'}", color=discord.Color.orange(), next_hint="Действие нельзя отменить.")

        async def _confirm(i: discord.Interaction) -> None:
            await member.kick(reason=reason)
            await self._log_action(interaction.guild.id, "kick", interaction.user.id, member.id, reason)
            await i.response.edit_message(content="✅ Пользователь кикнут.", embed=None, view=None)

        await interaction.response.send_message(embed=embed, view=ConfirmView(author_id=interaction.user.id, on_confirm=_confirm), ephemeral=True)

    @app_commands.command(name="ban", description="Забанить пользователя")
    @app_commands.checks.has_permissions(ban_members=True)
    async def ban(self, interaction: discord.Interaction, member: discord.Member, reason: str = "") -> None:
        embed = build_ux_embed(title="🚫 Подтвердите ban", description=f"Пользователь: {member.mention}\nПричина: {reason or 'не указана'}", color=discord.Color.red(), next_hint="Пользователь будет удален с сервера.")

        async def _confirm(i: discord.Interaction) -> None:
            await member.ban(reason=reason)
            await self._log_action(interaction.guild.id, "ban", interaction.user.id, member.id, reason)
            await i.response.edit_message(content="✅ Пользователь забанен.", embed=None, view=None)
    ...
    async def purge(self, interaction: discord.Interaction, limit: int) -> None:
        embed = build_ux_embed(title="🧹 Подтверждение purge", description=f"Будет удалено до **{limit}** последних сообщений в канале.", color=discord.Color.orange(), next_hint="Подтвердите, чтобы очистка началась.")

        async def _confirm(i: discord.Interaction) -> None:
            await interaction.channel.purge(limit=limit)
            await self._log_action(interaction.guild.id, "purge", interaction.user.id, reason=f"limit={limit}")
            await i.response.edit_message(content=f"✅ Удалено до {limit} сообщений.", embed=None, view=None)
    ...
    async def clearwarns(self, interaction: discord.Interaction, member: discord.Member) -> None:
        async with self.bot.db.session() as session:
            rows = await session.execute(select(Warning).where((Warning.guild_id == interaction.guild.id) & (Warning.user_id == member.id)))
            count = 0
            for warn in rows.scalars().all():
                await session.delete(warn)
                count += 1
            await session.commit()
        await self._log_action(interaction.guild.id, "clearwarns", interaction.user.id, member.id)
        await interaction.response.send_message(f"Удалено предупреждений: {count}", ephemeral=True)
    ...
    async def slowmode(self, interaction: discord.Interaction, seconds: int) -> None:
        ...
        await self._log_action(interaction.guild.id, "slowmode", interaction.user.id, reason=f"seconds={seconds}")
    ...
    async def modlog(self, interaction: discord.Interaction) -> None:
        async with self.bot.db.session() as session:
            rows = await session.execute(select(ModLog).where(ModLog.guild_id == interaction.guild.id).order_by(ModLog.created_at.desc()).limit(10))
            logs = rows.scalars().all()
        if not logs:
            await interaction.response.send_message("Лог пуст.", ephemeral=True)
            return
        embed = discord.Embed(title="ModLog", description="\n".join([f"{log.created_at.date()} {log.action} {log.user_id or ''} {log.reason}" for log in logs]))
        await interaction.response.send_message(embed=embed, ephemeral=True)
